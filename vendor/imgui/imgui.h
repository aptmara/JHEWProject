...
// preferred type. Add '#define IMGUI_DEFINE_MATH_OPERATORS' in your imconfig.h file to benefit from courtesy maths
...
namespace ImGui
{
    ...
    IMGUI_API void ShowIDStackToolWindow(bool* p_open = NULL); // create Stack Tool window. hover items with mouse to
    ...
    IsWindowHovered(ImGuiHoveredFlags flags =
                        0); // is current window hovered and hoverable (e.g. not blocked by a popup/modal)? See
                            ...
                            // function! Use the 'io.WantCaptureMouse' boolean for that! Refer to FAQ entry "How can I
    ...
    IMGUI_API ImVec2 GetWindowPos();   // get current window position in screen space (note: it is unlikely you need to
                                       // use this. Consider using current layout pos instead, GetCursorScreenPos())
    IMGUI_API ImVec2 GetWindowSize();  // get current window size (note: it is unlikely you need to use this. Consider
    ...
    GetWindowContentRegionMax(); // content boundaries max for the full window (roughly (0,0)+Size-Scroll) where Size
                                 // can be overridden with SetNextWindowContentSize(), in window coordinates
    ...
    PushTextWrapPos(float wrap_local_pos_x =
                        0.0f); // push word-wrapping position for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to
                               // end of window (or column); > 0.0f: wrap at 'wrap_pos_x' position in window local space
    ...
    IMGUI_API ImU32 GetColorU32(ImU32 col, float alpha_mul = 1.0f); // retrieve given color with style alpha applied,
    ...
    // from window-local to absolute coordinates.
    IMGUI_API ImVec2 GetCursorScreenPos(); // cursor position in absolute coordinates (prefer using this, also more
    ...
    IMGUI_API void
    EndGroup(); // unlock horizontal starting position + capture the whole group bounding box into one "item" (so you
    ...
    IMGUI_API float
    GetTreeNodeToLabelSpacing(); // horizontal distance preceding label when using TreeNode*() or Bullet() ==
    ...
    TableGetHoveredColumn(); // return hovered column. return -1 when table is not hovered. return columns_count if the
                             // unused space at the right of visible columns is hovered. Can also use
    ...
    SetNextItemAllowOverlap(); // allow next item to be overlapped by a subsequent item. Useful with invisible buttons,
                               // selectable, treenode covering an area where subsequent items may need to be added.
    ...
    IMGUI_API void
    SetStateStorage(ImGuiStorage* storage); // replace current window storage with our own (if you want to manipulate it
    ...
    //  512) according to how your backend/engine stored them in io.KeysDown[], but need to cast them to ImGuiKey.
    //  - with    IMGUI_DISABLE_OBSOLETE_KEYIO (this is the way forward): any use of ImGuiKey will assert with key <
    //  512. GetKeyIndex() is pass-through and therefore deprecated (gone if IMGUI_DISABLE_OBSOLETE_KEYIO is defined).
    ...
    IMGUI_API void
    SaveIniSettingsToDisk(const char* ini_filename); // this is automatically called (if io.IniFilename is not empty) a
...
// (Legacy: bit 0 must always correspond to ImGuiChildFlags_Border to be backward compatible with old API using 'bool
...
enum ImGuiChildFlags_
{
    ...
    ImGuiChildFlags_NavFlattened = 1 << 8, // Share focus scope, allow gamepad/keyboard navigation to cross over parent
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackCharFilter =
        1 << 20, // Callback on character inputs to replace or discard them. Modify 'EventChar' to replace or discard,
                 // or return 1 in callback to discard.
    ImGuiInputTextFlags_CallbackResize =
        1 << 21, // Callback on buffer capacity changes request (beyond 'buf_size' parameter value), allowing the string
    ...
    ImGuiInputTextFlags_CallbackEdit =
        1 << 22, // Callback on any edit (note that InputText() already returns true on edit, the callback is useful
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere = 1 << 15, // (WIP) Nav: left direction may move to this TreeNode() from any
...
struct ImDrawList
{
    ...
    // - Concave polygon fill is more expensive than convex one: it has O(N^2) complexity. Provided as a convenience fo
    // user but not used by main library.
...
namespace ImGui
{
    ...
    GetKeyIndex(ImGuiKey key); // Map ImGuiKey_* values into legacy native key index. == io.KeyMap[key]. When using
                               // a 1.87+ backend using io.AddKeyEvent(), calling GetKeyIndex() with ANY ImGuiKey_XXXX
    ...
    // SetNextFrameWantCaptureKeyboard(want_capture_keyboard); } // Renamed as name was misleading + removed default
    // value. static inline void  CaptureMouseFromApp(bool want_capture_mouse = true)               {
    // SetNextFrameWantCaptureMouse(want_capture_mouse); }       // Renamed as name was misleading + removed default
    // value.
    //-- OBSOLETED in 1.86 (from November 2021)
    // IMGUI_API void      CalcListClipping(int items_count, float items_height, int* out_items_display_start, int*
    // out_items_display_end); // Code removed, see 1.90 for last version of the code. Calculate range of visible items
    // for large list of evenly sized items. Prefer using ImGuiListClipper.
    ...
    // ImMin(items_count, 7) : height_in_items) + 0.25f) + GetStyle().FramePadding.y * 2.0f; return BeginListBox(label,
    // ImVec2(0.0f, height)); } // Helper to calculate size from items_count and height_in_items static inline void
    ...
    // DragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void*
    // p_min, const void* p_max, const char* format, float power = 1.0f);                                          //
    // OBSOLETED in 1.78 (from June 2020) IMGUI_API bool      SliderScalar(const char* label, ImGuiDataType data_type,
    // void* p_data, const void* p_min, const void* p_max, const char* format, float power = 1.0f); // OBSOLETED in 1.78
    // (from June 2020) IMGUI_API bool      SliderScalarN(const char* label, ImGuiDataType data_type, void* p_data, int
    // components, const void* p_min, const void* p_max, const char* format, float power = 1.0f); // OBSOLETED in 1.78
    // (from June 2020) static inline bool  DragFloat(const char* label, float* v, float v_speed, float v_min, float
    // v_max, const char* format, float power = 1.0f)    { return DragScalar(label, ImGuiDataType_Float, v, v_speed,
    // &v_min, &v_max, format, power); }     // OBSOLETED in 1.78 (from June 2020) static inline bool  DragFloat2(const
    // char* label, float v[2], float v_speed, float v_min, float v_max, const char* format, float power = 1.0f) {
    // return DragScalarN(label, ImGuiDataType_Float, v, 2, v_speed, &v_min, &v_max, format, power); } // OBSOLETED
    // in 1.78 (from June 2020) static inline bool  DragFloat3(const char* label, float v[3], float v_speed, float
    // v_min, float v_max, const char* format, float power = 1.0f) { return DragScalarN(label, ImGuiDataType_Float, v,
    // 3, v_speed, &v_min, &v_max, format, power); } // OBSOLETED in 1.78 (from June 2020) static inline bool
    // DragFloat4(const char* label, float v[4], float v_speed, float v_min, float v_max, const char* format, float
    // power = 1.0f) { return DragScalarN(label, ImGuiDataType_Float, v, 4, v_speed, &v_min, &v_max, format, power); }
    // // OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat(const char* label, float* v, float v_min,
    // float v_max, const char* format, float power = 1.0f)                 { return SliderScalar(label,
    // ImGuiDataType_Float, v, &v_min, &v_max, format, power); }            // OBSOLETED in 1.78 (from June 2020) static
    // inline bool  SliderFloat2(const char* label, float v[2], float v_min, float v_max, const char* format, float
    // power = 1.0f)              { return SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min, &v_max, format,
    // power); }        // OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat3(const char* label, float
    // v[3], float v_min, float v_max, const char* format, float power = 1.0f)              { return
    // SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format, power); }        // OBSOLETED in 1.78
    // (from June 2020) static inline bool  SliderFloat4(const char* label, float v[4], float v_min, float v_max, const
    // char* format, float power = 1.0f)              { return SliderScalarN(label, ImGuiDataType_Float, v, 4, &v_min,
    ...
    // }            // OBSOLETED in 1.60 (from Apr 2018) static inline bool  IsAnyWindowHovered()                  {
    // return IsWindowHovered(ImGuiHoveredFlags_AnyWindow); }            // OBSOLETED in 1.60 (between Dec 2017 and Apr
    // 2018) static inline void  ShowTestWindow()                      { return ShowDemoWindow(); } // OBSOLETED in 1.53
    ...
    // 2017) static inline void  SetNextWindowPosCenter(ImGuiCond c=0) {
    // SetNextWindowPos(GetMainViewport()->GetCenter(), c, ImVec2(0.5f,0.5f)); } // OBSOLETED in 1.52 (between Aug 2017
    // and Oct 2017) static inline bool  IsItemHoveredRect()                   { return
    ...
    // ImFont*GetWindowFont()                      { return GetFont(); } // OBSOLETED in 1.48 static inline float
    // GetWindowFontSize()                   { return GetFontSize(); }                                           //
    // OBSOLETED in 1.48 static inline void  SetScrollPosHere()                    { SetScrollHere(); } // OBSOLETED
    // in 1.42
} // namespace ImGui
